...
async function confirmSelection() {
    try {
        ...
        const response = await fetch('セキュリティ上の理由で削除しました。:8006/image', {
            method: 'POST',
            headers: {
...
async function goToLoading() {
    ...
    try {
        ...
        const response = await fetch('セキュリティ上の理由で削除しました。:8006/cafeList', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function downloadImage() {
    try {
        // バックエンドから生成された画像を取得
        const response = await fetch('セキュリティ上の理由で削除しました。:8006/image'); // バックエンドから実際の画像ファイルを取得するAPI
